...
public class Coll<E extends Entity<E>> extends CollAbstract<E>
{
	...
	public synchronized void loadFromRemoteFixed(String id, Entry<JsonObject, Long> remoteEntry)
	{
		...
		{
			logLoadError(id, String.format("ão é possível carregar a entidade da versão %d", version));
			return;
		}
	...
	public boolean remoteEntryIsOk(String id, Entry<JsonObject, Long> remoteEntry)
	{
		...
		if (raw == null)
		{
			this.logLoadError(id, "Os dados brutos eram nulos. O arquivo esta completamente vazio?");
			return false;
		}
		if (raw.isJsonNull())
		{
	...
	{
		MassiveCore.get().log(Txt.parse("<b>O banco de dados nao pode carregar a entidade. Voce editou um arquivo manualmente e fez o JSON errado?"));
	...
	{
		if (id == null) throw new NullPointerException("id");
		if (!local && !remote) throw new IllegalArgumentException("Deve ser remoto ou local.");
	...
	public MassivePlugin calculatePlugin()
	{
		...
		// Return
		if (ret == null) throw new RuntimeException("o plugin nao pode ser calculado");
		return ret;
	}
