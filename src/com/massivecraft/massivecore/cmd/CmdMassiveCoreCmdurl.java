...
public class CmdMassiveCoreCmdurl extends MassiveCoreCommand
{
	...
	{
		// Parameters
		this.addParameter(TypeString.get(), "url").setDesc("a url para carregar");
	...
	public void perform() throws MassiveException
	{
		...
		{
			msg("<b>URL malformada: %s", e.getMessage());
			return;
		}
		
		// Apply 
		final Player commander = me;
		msg("<i>Carregando <aqua>%s <i>...", urlString);
		async(new Runnable()
		{
			@Override
			public void run()
			{
				try
				{
					...
					sync(new Runnable()
					{
						...
						public void run()
						{
							MixinMessage.get().msgOne(commander, "<i>... <h>%d <i>linhas carregadas. Executando ...", lines.size());
