...
public class EngineMassiveCoreCommandSet extends Engine
{
	...
	public <T extends Serializable> void setValue(EventMassiveCoreCommandSet<T> event) throws MassiveException
	{
		...
		if (after == before)
		{
			throw new MassiveException().addMsg("%s<i> já está <gold>%s<i>.", targetDesc, afterDesc);
		}
		...
		// Inform
		MixinMessage.get().msgOne(senderId, "%s<i> agora é <gold>%s<i>.", targetDesc, afterDesc);
		
		// Inform target
		if (!targetId.equals(senderId))
		{
			MixinMessage.get().msgOne(targetId, "%s<i> agora é <gold>%s<i>.", getTargetDesc(targetId, targetId, name), afterDesc);
		}
	}
	...
	public String getTargetDesc(String targetId, String watcherId, String name)
	{
		...
		{
			return Txt.parse("<i>Seu <gold>%s", name);
		}
		else
		{
			return Txt.parse("<i>%s de %s", Txt.upperCaseFirst(name), MixinDisplayName.get().getDisplayName(targetId, watcherId));
		}
	}
