...
public class MassiveCommand implements Active, PluginIdentifiableCommand
{
	...
	// Use these requirements must be met for the command to be executable;
	...
	// A base prefix such as "CmdFactions" that use names of commands in a plugin start with.
	// Used for finding the right permission.
	// Should be set in a super class for use the commands in a plugin.
	...
	public static final Mson TEMPLATE_CORE = Mson.mson("/").color(ChatColor.GOLD);
	...
	public Mson getTemplate(boolean addDesc, boolean onlyFirstAlias, CommandSender sender)
	{
		...
		for (MassiveCommand command : commands)
		{
			...
			if ( ! first) ret = ret.add(Mson.EMPTY);
			ret = ret.add(mson); 
			first = false;
		}
	...
	public List<String> getTabCompletions(List<String> args, CommandSender sender)
	{
		...
		if (sender == null) throw new IllegalArgumentException("§co rementente é nulo");
