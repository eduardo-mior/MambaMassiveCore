...
public abstract class MassiveCommandSet<T> extends MassiveCommand
{
	...
	public String getTargetDesc(String targetId, String watcherId)
	{
		...
		{
			return Txt.parse("<i>Seu <h>%s", this.getName());
		}
		else
		{
			return Txt.parse("<h>%s <i>do %s", Txt.upperCaseFirst(this.getName()), MixinDisplayName.get().getDisplayName(targetId, watcherId));
		}
	}
	...
	public void perform() throws MassiveException
	{
		...
		if (after == before)
		{
			throw new MassiveException().addMsg("%s<i> já está <h>%s<i>.", targetDesc, afterDesc);
		}
		...
		// Inform
		msg("%s<i> é gora <h>%s<i>.", targetDesc, afterDesc);

		// Inform target
		if (!targetId.equals(senderId))
		{
			MixinMessage.get().msgOne(targetId, "%s<i> é agora <h>%s<i>.", getTargetDesc(targetId, targetId), afterDesc);
		}
	}
