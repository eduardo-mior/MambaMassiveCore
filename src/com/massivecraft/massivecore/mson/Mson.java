...
public class Mson implements Serializable
{
	...
	Mson(String text, ChatColor color, Boolean bold, Boolean italic, Boolean underlined, Boolean strikethrough, Boolean obfuscated, MsonEvent clickEvent, MsonEvent hoverEvent, String insertionString, List<Mson> extra, Mson parent)
	{
		...
		// Color
		if (color != null && ! color.isColor()) throw new IllegalArgumentException(color.name() + " não é uma cor.");
		this.color = color;
		...
		if (event != null && event.getType() != type) throw new IllegalArgumentException(event.getAction().name() + " não é um tipo de " + type);
	...
	private static Mson getMson(Object part)
	{
		...
		{
			throw new IllegalArgumentException("Nos aceitamos apenas Strings, Msons, Collections e Arrays.");
		}
	}
	...
	public static Mson fromJson(JsonElement json)
	{
		...
		throw new IllegalArgumentException("Nem string nem object: " + json);
	}
